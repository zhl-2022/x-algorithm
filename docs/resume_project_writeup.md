...
- 在 KuaiRec big 上完成 hard negative、in-batch negative、ItemCF 蒸馏、LightGCN、序列兴趣模型和轻量文本 encoder 对比，并通过蒸馏召回 + Ranker pipeline 精调将神经 `NDCG@20` 提升到 `0.055883`，换 seed 复跑仍达到 `0.052947`。
...
1. 若继续扩大规模，迁移到 Tenrec 或 KuaiRand，验证当前推荐 pipeline 在更大信息流数据上的可迁移性。
