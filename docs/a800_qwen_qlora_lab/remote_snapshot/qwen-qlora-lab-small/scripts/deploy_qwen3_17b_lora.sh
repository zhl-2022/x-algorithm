...
CONTAINER_NAME="${CONTAINER_NAME:-qwen3-17b-lora-swift-deploy}"
PORT="${1:-${QWEN3_LORA_PORT:-18081}}"
MODEL_DIR="${MODEL_DIR:-$PROJECT_DIR/models/Qwen3-1.7B}"
ADAPTER_DIR="${ADAPTER_DIR:-$PROJECT_DIR/outputs/qwen3_17b_text/v1-20260613-211241/checkpoint-20}"
...
if [ ! -f "$ADAPTER_DIR/adapter_config.json" ]; then
  echo "Missing adapter config: $ADAPTER_DIR/adapter_config.json" >&2
  exit 1
fi
...
nvidia-smi > "$LOG_DIR/qwen3_17b_lora_deploy.before.nvidia-smi.txt"
...
docker run -d \
  ...
  bash -lc "swift deploy \
    ...
    --host 0.0.0.0 \
    --port ${PORT} \
    --served_model_name qwen3-1.7b-qlora-round1 \
    --max_length 2048 \
    --max_new_tokens 512 \
    --temperature 0.2 \
    --stream false" \
  > "$LOG_DIR/qwen3_17b_lora_deploy.container_id.txt"

echo "Waiting for http://127.0.0.1:${PORT}/v1/models ..."
for _ in $(seq 1 90); do
  if curl -fsS "http://127.0.0.1:${PORT}/v1/models" > "$LOG_DIR/qwen3_17b_lora_deploy.models.json"; then
    nvidia-smi > "$LOG_DIR/qwen3_17b_lora_deploy.after.nvidia-smi.txt"
    echo "Qwen3-1.7B LoRA deploy is ready on port ${PORT}."
    docker ps --filter "name=^/${CONTAINER_NAME}$"
    exit 0
  fi
  sleep 2
