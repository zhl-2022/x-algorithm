...
- [X] 打开 `training_status.log`。
- [X] 找到第一次失败。
- [X] 找到第一次成功。
- [X] 找出三轮最终 checkpoint。
- [X] 用一句话说明显存增量是否在可接受范围内。
...
- [X] 打开 `round1_qwen3_17b_text.train.log`。
- [X] 搜索 `Saving model checkpoint`。
- [X] 搜索 `train_runtime`。
- [X] 找到 `last_model_checkpoint`。
...
- [X] 打开 `infer_rounds_inside.sh`。
- [X] 找到 `--adapters`。
- [X] 打开 `round1_qwen3_17b_text.infer.md`。
- [X] 读两条 `result_preview`。
- [X] 写一句“这次推理验证有什么不足”。
...
- [X] 在 WebUI 找到 `模型 id 或路径`。
- [X] 在 WebUI 找到 `数据集名称`。
- [X] 在 WebUI 找到 `max_length`。
- [X] 在 WebUI 找到 `lora_rank`。
- [X] 在 WebUI 找到 `quant_bits`。
- [X] 不看文档，自己解释这 5 个字段。
...
- [X] 读 `data/text_sft.jsonl`，解释 `messages`。
- [X] 读 `scripts/train_rounds_inside.sh`，解释 10 个核心参数。
- [X] 读 `outputs/qwen3_17b_text/.../args.json`，找到实际参数。
- [X] 读 `outputs/qwen3_17b_text/.../checkpoint-10/adapter_config.json`，解释 LoRA。
- [X] 读 `logs/round1_qwen3_17b_text.infer.md`，解释推理验证。
...
- [X] 打开 `ms-swift` WebUI。
- [X] 在 `LLM预训练/微调` 页找到模型设置。
- [X] 对照 `train_rounds_inside.sh` 填入相同参数。
- [X] 不点击训练，只截图或记录你填了哪些字段。
