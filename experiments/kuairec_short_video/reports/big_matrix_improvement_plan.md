...
| Pipeline `NDCG@20` | 已达到 `0.048158`，高于 stage5 best pipeline 的 `0.005245` |
...
4. Pipeline 验证：用蒸馏 Two-Tower 作为召回底座，再接 DNNRanker hard negative 重排，当前最佳 `NDCG@20=0.048158`。
...
KuaiRec big 的下一步应该从“继续堆同类神经模型”转为“让神经召回学习强协同信号”。ItemCF 蒸馏 Two-Tower 是最直接的补强方向：它既利用了当前最强的 ItemCF 结果，又保留了 Two-Tower 适合向量召回和后续工程化的优势。阶段九已经证明，蒸馏不是 teacher 越多越好，后续更应该优化 teacher soft label 和随机负样本比例。
