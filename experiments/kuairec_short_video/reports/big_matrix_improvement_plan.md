...
2026-06-03 已完成三组蒸馏 pipeline 精调，核心变量是 teacher 样本比例和随机负样本比例。

| 实验 | 最佳模型 | NDCG@20 | 判断 |
|---|---|---:|---|
| `distill_pipeline_800k_t40n40` | `DistillTwoTower+DNN-Rerank@200` | 0.039138 | 未超过阶段八 |
| `distill_pipeline_2m_t40n120` | `DistillTwoTower+DNN-Rerank@100` | 0.048158 | 当前最佳神经 pipeline |
| `distill_pipeline_2m_t120n40` | `DistillTwoTower+DNN-Blend@100a0.5` | 0.039845 | teacher 占比过高后 TopK 不足 |

阶段九结论：降低 teacher 占比、提高随机负样本比例比单纯增加 teacher 样本更有效。
当前应以 `2m_t40n120` 为主线，继续做 teacher soft label 和负样本比例消融。

## 2.4 阶段十和阶段十一执行结果

2026-06-03 已完成 KuaiRec 最终收尾实验。阶段十修正了蒸馏负样本采样逻辑，让
`negative_items_per_user` 真正参与每用户随机负样本上限，并加入 teacher soft label 变换。

| 阶段 | 实验 | 最佳模型 | NDCG@20 | 判断 |
|---|---|---|---:|---|
| Stage10 | `soft_replay_p29_t14_linear` | `Blend@100a0.75` | 0.051595 | 超过阶段九 |
| Stage10 | `soft_p30_t10_linear` | `Blend@100a0.5` | 0.052608 | 达到强成功线 |
| Stage10 | `soft_p30_t15_linear` | `Rerank@100` | 0.053271 | 继续提升 |
| Stage10 | `soft_p30_t15_sqrt` | `Blend@100a0.5` | 0.055883 | 当前最佳神经 pipeline |
| Stage11 | `final_replay_soft_p30_t15_sqrt_seed2027` | `Blend@100a0.5` | 0.052947 | 换 seed 后仍超过阶段九 |

最终判断：KuaiRec big 神经 pipeline 已从 Stage5 `0.005245` 提升到 Stage10 `0.055883`，
复跑仍有 `0.052947`。当前数据集模型实验收尾，不再继续堆新模型。
...
| Pipeline `NDCG@20` | 已达到 `0.055883`，高于 stage5 best pipeline 的 `0.005245` |
| 与 ItemCF 差距 | 已明显缩小，但仍低于 ItemCF `0.065921` |
...
4. Pipeline 验证：用蒸馏 Two-Tower 作为召回底座，再接 DNNRanker hard negative 重排，当前最佳 `NDCG@20=0.055883`。
...
KuaiRec big 的下一步已经从“继续堆同类神经模型”转为“让神经召回学习强协同信号”。ItemCF 蒸馏 Two-Tower 是最直接的补强方向：它既利用了当前最强的 ItemCF 结果，又保留了 Two-Tower 适合向量召回和后续工程化的优势。阶段十和阶段十一完成后，本数据集模型实验收尾，后续重点转向项目总结或第四批数据集。
